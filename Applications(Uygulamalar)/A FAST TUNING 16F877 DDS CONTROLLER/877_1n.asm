...
;
; CALIBRATE MODE is entered if the external push button is pressed during
; power on. The display is set to 10,000.000 CAL and remains fixed, even as
; adjustments are being made. If the push button is held pressed, then turning
; the shaft encoder will increase or decrease the value "osc" used to 
...
; while turning.  An external frequency counter on the DDS output is required
; to observe this adjustment. To exit calibrate mode, release the external
; push button and turn the shaft encoder one more time. The calibrated value
; of "osc" will then be stored in EEPROM memory.
;
; IF OFFSET is added or subtracted depending upon state of RA4. This operates 
...
;		7	D7
;
;	Port C	0	LCD_rs  equ     0x00              ; 0=instruction, 1=data
;		1	LCD_rw  equ     0x01              ; 0=write, 1=read
;		2	LCD_e   equ     0x02              ; 0=disable, 1=enable
;		3	DDS_load equ    0x03              ; Update pin on AD9850
;		4	DDS_clk equ     0x04              ; AD9850 write clock
;		5	DDS_dat equ     0x05              ; AD9850 serial data input
;		6	Bootloader Rx (Keep Free)
;		7	Bootloader Tx (Keep Free)
;
;	Port D	0	*RIT Line, Active Low
;		1	*Fine Tune, High 10Hz, Low 1Hz
;		2	*TX (High) RX (Low) Line
...
DDS_clk 	equ     0x04              ; AD9850 write clock
DDS_data 	equ     0x05              ; AD9850 serial data input

; Port D


RIT_State	equ	0x00		  ; RIT High Off Low on
Tune_Rate	equ	0x01		  ; High 10Hz, Low 1Hz
TR_Line		equ	0x02		  ; Tx High Rx Low Line
band_ch 	equ 	0x03              ; Calibrate Push Button, (Active Low)
...
; DDS Variables
	...
	cntl_read		  ; PortD switch state 
	cntl_new		  ; PortD switch new state
	cntl_old		  ; PortD switch old state	
...
;
; This will not affect operation without a Bootloader.
;
; ****************************************************************************
;
	nop
...
; Configure Ports
	...
	Bank1			  ; Switch to bank 1
        ...
        movlw 	0x00		  ; Set Port C as outputs
	movwf   TRISC		  ; 
	movlw	0x07		  ; Set Port D as inputs
	movlw	TRISD		  ;
...
; A to D set up
	Bank1			  ; Switch to bank 1
	movlw	b'00111111'
	movwf	TRISA		  ; PortA Inputs for A to D
	movlw	b'00000100'
	movwf	ADCON1		  ; Left justify, 3 analog channel, no Vref 
...
;       power on.
        ...
        btfsc   PortD,band_ch	  ; Is the calibrate function selected?
...
;
        call    IF_Offset	  ; Adjust base freq data for IF	
...
; S-Meter Update
	...
	goto	check_controls	  ; Not time to update S-Meter, move onto IF checks
...
check_controls			  ; Check state of Port D Controls
	...
	call	change_band	  ; Check Band Switch state
...
; Fine tuning routine
	...
	goto	Fine_tune	  ; Yes select Fine Tune
        btfsc   PortD,1        	  ; Is fine tune selected?
...
change_band
	...
	btfsc   PortD,band_ch     ; Band Change function selected?
	return			  ; No, return to main 
	call	read_encoder
...
valid
	...
	goto	change_band       ; Set the frequency and continue
band_up
        ...
        call    get_band          ; Get the new band frequency
	goto	change_band       ; Continue polling the encoder (endless loop)
...
RIT_Tx				  ; Save RIT freq and Restore base freq during Tx
	btfss	PortD,TR_Line	  ; Are we in Tx mode? (0=Tx)
...
;bit 0        	
		rrf     byte2send,f       ; Test if next bit is 1 or 0
        	...
        	bsf     PortC,DDS_data    ; No, send one
		goto	sendbit0
set00   	bcf     PortC,DDS_data    ; Send zero
sendbit0        bsf     PortC,DDS_clk     ; Toggle write clock
        	bcf     PortC,DDS_clk     ;
;bit 1
        	rrf     byte2send,f       ; Test if next bit is 1 or 0
        	btfss   STATUS,B_C        ; Was it zero?
        	...
        	bsf     PortC,DDS_data    ; No, send one
		goto	sendbit1
set01   	bcf     PortC,DDS_data    ; Send zero
sendbit1        bsf     PortC,DDS_clk     ; Toggle write clock
        	bcf     PortC,DDS_clk     ;
;bit 2
        	rrf     byte2send,f       ; Test if next bit is 1 or 0
        	btfss   STATUS,B_C        ; Was it zero?
        	...
        	bsf     PortC,DDS_data    ; No, send one
		goto	sendbit2
set02   	bcf     PortC,DDS_data    ; Send zero
sendbit2        bsf     PortC,DDS_clk     ; Toggle write clock
        	bcf     PortC,DDS_clk     ;
;bit 3
        	rrf     byte2send,f       ; Test if next bit is 1 or 0
        	btfss   STATUS,B_C        ; Was it zero?
        	...
        	bsf     PortC,DDS_data    ; No, send one
		goto	sendbit3
set03   	bcf     PortC,DDS_data    ; Send zero
sendbit3        bsf     PortC,DDS_clk     ; Toggle write clock
        	bcf     PortC,DDS_clk     ;
;bit 4
        	rrf     byte2send,f       ; Test if next bit is 1 or 0
        	btfss   STATUS,B_C        ; Was it zero?
        	...
        	bsf     PortC,DDS_data    ; No, send one
		goto	sendbit4
set04	  	bcf     PortC,DDS_data    ; Send zero
sendbit4        bsf     PortC,DDS_clk     ; Toggle write clock
        	bcf     PortC,DDS_clk     ;
;bit 5
        	rrf     byte2send,f       ; Test if next bit is 1 or 0
        	btfss   STATUS,B_C        ; Was it zero?
        	...
        	bsf     PortC,DDS_data    ; No, send one
		goto	sendbit5
set05    	bcf     PortC,DDS_data    ; Send zero
sendbit5        bsf     PortC,DDS_clk     ; Toggle write clock
        	bcf     PortC,DDS_clk     ;
;bit 6
        	rrf     byte2send,f       ; Test if next bit is 1 or 0
        	btfss   STATUS,B_C        ; Was it zero?
        	...
        	bsf     PortC,DDS_data    ; No, send one
		goto	sendbit6
set06   	bcf     PortC,DDS_data    ; Send zero
sendbit6        bsf     PortC,DDS_clk     ; Toggle write clock
        	bcf     PortC,DDS_clk     ;
;bit 7
        	rrf     byte2send,f       ; Test if next bit is 1 or 0
        	btfss   STATUS,B_C        ; Was it zero?
        	...
        	bsf     PortC,DDS_data    ; No, send one
		goto	sendbit7
set07   	bcf     PortC,DDS_data    ; Send zero
sendbit7        bsf     PortC,DDS_clk     ; Toggle write clock
        	bcf     PortC,DDS_clk     ;
        ...
        btfss   STATUS,B_C        ;
        goto    next_byte         ;
        bsf     PortC,DDS_load    ; Send load signal to the AD9850
        bcf     PortC,DDS_load    ; 
        return                    ;
...
;
IF_Offset			
	btfss	PortD,IF_Offset_Use
	return				; IF Offset not required, return to caller
	...
	movwf	DDS_freq_3
	 ...
	 btfss 	PortD,5		; Test state of IF offset switch
...
add_IF
        movlw   IF_Add_0           ; Get low byte of the increment
...
IF_comp_done
;
;	Now add the compliment of DDS_freq and IF
;
	movlw   IF_Add_0           ; Get low byte of the increment
        addwf   DDS_freq_0,f       ; Add it to the low byte of freq
        btfss   STATUS,B_C         ; Any carry?
...
; *****************************************************************************
; *                                                                           *
; * Purpose:  This routine is entered at start up if the push button is       *
...
add6
        ...
        btfss   PortD,band_ch     ; Is the band change function still selected?
