...
; RIT function
;
; 2b changes;	Band control for filter selection
...
;		2	*Tuning Lock
;
;	Band Frequencies and output codes on Port D 3..7
;
...
; Port B 	LCD Controls and 4 bit Data
...
band_table			  ; 		Band Indicator (Port A 2..5)
        addwf   PCL,f             ; 
...
;
	...
	call	Start_delay	  ; Show message for 1 Sec to allow control lines to settle
;
; A to D set up
;
	Bank1			  ; Switch to bank 1
	...
	movwf	ADCON1
	movlw	b'00000011'
	movwf	TRISA		  ; PortA 0..1 Inputs for A to D, 2..5 Outputs for filter selection
...
;       power on.
        ...
        btfsc   PortC,band_ch	  ; Is the calibrate function selected?
...
read_EEocs
	...
	Bank0
        movwf   osc_3             ; Save it
        call    read_EEPROM       ; Get last used band word
	movf    EEdata,w          ;
	Bank0
        movwf   band              ; Save it
...
; * On de-selection of band change, new value of band word is written to      *
; * EEPROM for power on configuration                                         *
; *****************************************************************************
change_band
...
;
; Running 4 bit mode, split BCD into bytes into nibbles and send MSB
; part first.
;
...
LCD_is_busy
	...
	nop
	nop
	movf	PortB,w		  ; Save PortB high nibble
	movwf	LCD_read	  ; for testing
	nop
	nop
	...
	nop
        ...
        btfsc   LCD_read,7        ; Is Busy Flag (RC7) in saved byte clear?
	goto LCD_is_busy
LCD_is_free
	bcf	PortB,LCD_rw
        return                    ; 
...
;
	...
	nop
	nop
	iorwf   PortB             ; Send to RB7..RB4 without changing RB3..RB0
	nop
	nop                       ;   wait, 
        bcf     PortB,LCD_e       ;   and drop it again
        return
...
Start_delay		          ; Long wait (1 Sec) to allow any start up control
